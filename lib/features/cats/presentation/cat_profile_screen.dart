...
/// Kedi Profili - Kullanışlı bakım takip ekranı
...
class _CatProfileScreenState extends ConsumerState<CatProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final catReminders = allReminders.where((r) => r.catId == _currentCat.id).toList();
      ...
      actions: [
        IconButton(
          ...
          onPressed: () async {
            ...
            if (updated != null) setState(() => _currentCat = updated);
          },
        ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: CircleAvatar(
                      ...
                      backgroundImage: _photoExists(_currentCat.photoPath)
                          ? (_currentCat.photoPath!.startsWith('http')
                              ? CachedNetworkImageProvider(_currentCat.photoPath!)
                              : FileImage(File(_currentCat.photoPath!)) as ImageProvider)
                          : null,
                      child: !_photoExists(_currentCat.photoPath)
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _buildInfoChip(Icons.cake_outlined, DateHelper.getAge(_currentCat.birthDate))
                                .animate()
          ...
          ...overdueTasks.take(3).map((task) => Padding(
            ...
            child: GestureDetector(
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => RecordDetailScreen(record: task, catName: _currentCat.name)),
              ),
              child: Container(
          ...
          Row(
            children: [
              _buildQuickActionButton(
                ...
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => WeightScreen(cat: _currentCat)),
                ),
                isDark: isDark,
              ),
              const SizedBox(width: 12),
              _buildQuickActionButton(
                ...
                onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(builder: (_) => AddReminderScreen(
                    preselectedCatId: _currentCat.id,
                    initialType: 'vaccine',
                  )),
                ),
                isDark: isDark,
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                '${_currentCat.name} için ilk hatırlatıcıyı ekleyerek başlayın',
                style: TextStyle(
                  fontSize: 13,
                  color: AppColors.textSecondary,
                ),
  ...
  Widget _buildRecordItem(Reminder reminder, bool isDark) {
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => RecordDetailScreen(record: reminder, catName: _currentCat.name)),
        );
      },
      child: Container(
        margin: const EdgeInsets.only(bottom: 8),
