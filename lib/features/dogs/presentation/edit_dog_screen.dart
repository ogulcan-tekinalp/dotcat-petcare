...
class _EditDogScreenState extends ConsumerState<EditDogScreen> {
  ...
  void initState() {
    ...
    _notesController = TextEditingController(text: widget.dog.notes ?? '');
    _birthDate = widget.dog.birthDate;
    _selectedGender = widget.dog.gender;
    _selectedSize = widget.dog.size;
    _savedPhotoPath = widget.dog.photoPath;
    ...
    try {
      ...
      if (_photoFile != null && _savedPhotoPath != null && !_savedPhotoPath!.startsWith('http')) {
        try {
          ...
          if (widget.dog.photoPath != null && widget.dog.photoPath!.startsWith('http')) {
            await StorageService.instance.deleteDogPhoto(widget.dog.id);
          }
          // Yeni fotoğrafı yükle
          final uploadedUrl = await StorageService.instance.uploadDogPhoto(_photoFile!, widget.dog.id);
          if (uploadedUrl != null) {
            photoUrl = uploadedUrl;
          }
        } catch (e) {
          debugPrint('EditDogScreen: Photo upload error: $e');
        }
      } else if (_savedPhotoPath == null && widget.dog.photoPath != null && widget.dog.photoPath!.startsWith('http')) {
        try {
          // Fotoğraf silindi, cloud'dan da sil
          await StorageService.instance.deleteDogPhoto(widget.dog.id);
        } catch (e) {
          debugPrint('EditDogScreen: Photo delete error: $e');
        }
      }
  ...
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final isUrl = _savedPhotoPath != null && _savedPhotoPath!.startsWith('http');

    return Scaffold(
      appBar: AppBar(title: Text(AppLocalizations.get('edit_dog'))),
      body: Form(
        key: _formKey,
        child: ListView(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: _photoFile != null
                        ...
                        : (isUrl
                            ? DecorationImage(image: NetworkImage(_savedPhotoPath!), fit: BoxFit.cover)
                            : null),
                  ),
                  child: (_photoFile == null && !isUrl)
